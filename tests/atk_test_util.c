...
AtspiAccessible * get_root_obj (const char *file_name)
{
  ...
  struct timespec timeout = { .tv_sec = 0, .tv_nsec = 100 * 1000000 };
  ...
  /* Wait for application to start, up to 10 times 100ms.  */
  while (++tries <= 10)
  {
...
void terminate_app (void)
{
  ...
  /* Wait for application to stop, up to 10 times 100ms.  */
  while (++tries <= 10)
  {
    child = try_get_root_obj (obj);
    if (child == NULL)
